defmodule Wik.Blocks.Types.TextTest do
  ...
  describe "block_to_form_params/3" do
    test "uses persisted text" do
      ...
      assert %{"text" => "Hello"} = Wik.Blocks.block_to_form_params(block, %{}, nil)
    end
  end
  ...
  describe "update_block/3" do
    test "stores submitted text" do
      ...
      {:ok, block} =
        Wik.Blocks.create_user_owned_block(
          %{type: :text},
          scope: scope
        )

      assert {:ok, updated_block} =
               Wik.Blocks.update_block(block, %{"text" => "Updated"}, scope: scope)
