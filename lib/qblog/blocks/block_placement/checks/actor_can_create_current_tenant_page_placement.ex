defmodule Qblog.Blocks.BlockPlacement.Checks.ActorCanCreateCurrentTenantPagePlacement do
  ...
  def match?(actor, %{subject: subject} = context, _opts) do
    ...
    with true <- attachable_type == "page",
         ...
         {:ok, %Page{group_id: ^group_id}} <-
           Ash.get(Page, attachable_id, authorize?: false, domain: Qblog.Wiki, tenant: tenant),
         {:ok, %Block{} = block} <-
           Ash.get(Block, block_id, authorize?: false, domain: Qblog.Blocks),
