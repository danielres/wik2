defmodule Wik.Blocks.Versioning do
  alias Ash.Query
  alias HSDiff
  ...
  def create_initial_text_version(block, current_text, opts) do
    scope = Keyword.fetch!(opts, :scope)
    create_version(block, 1, :snapshot, current_text, nil, scope)
  end

  def update_block_with_text_version(block, current_text, updated_text, updated_data, opts) do
    scope = Keyword.fetch!(opts, :scope)
    ...
    case Wik.Repo.transaction(fn ->
           with {:ok, updated_block, notifications} <-
                  block |> Ash.update(%{data: updated_data}, update_opts),
                :ok <-
                  create_next_version(block, current_text, updated_text, scope) do
             {updated_block, notifications}
           else
             {:error, error} -> Wik.Repo.rollback(error)
           end
         end) do
  ...
  def version_to_text(_block, %BlockVersion{storage_kind: :snapshot, snapshot_text: text}, _opts),
    do: {:ok, text || ""}

  def version_to_text(block, %BlockVersion{} = version, opts) do
    scope = Keyword.fetch!(opts, :scope)

    with {:ok, snapshot_version} <- get_snapshot_version(block, version, scope),
         {:ok, versions} <- list_versions_to_replay(block, version, snapshot_version, scope) do
      text =
        versions
  ...
  defp get_snapshot_version(block, version, scope) do
    BlockVersion
    |> Query.filter(
      block_id == ^block.id and
        block_type == ^block.type and
  ...
  defp list_versions_to_replay(block, version, snapshot_version, scope) do
    BlockVersion
    |> Query.filter(
      block_id == ^block.id and
        block_type == ^block.type and
  ...
  defp create_next_version(
         block,
         previous_text,
         updated_text,
         scope,
         attempts_left \\ @revision_retry_attempts
       )
  ...
  defp get_next_revision(block, scope) do
    BlockVersion
    |> Query.filter(block_id == ^block.id and block_type == ^block.type)
    |> Query.sort(revision: :desc)
    |> Query.limit(1)
    |> Ash.read(authorize?: false, scope: scope)
    |> case do
